package dev.joaoshiozo.insomnia.core.api.gui.clickgui.button;

import dev.joaoshiozo.insomnia.InsomniaMod;
import dev.joaoshiozo.insomnia.core.api.gui.clickgui.button.settings.BindButton;
import dev.joaoshiozo.insomnia.core.api.gui.clickgui.button.settings.BoolButton;
import dev.joaoshiozo.insomnia.core.api.gui.clickgui.button.settings.EnumButton;
import dev.joaoshiozo.insomnia.core.api.gui.clickgui.button.settings.SliderButton;
import dev.joaoshiozo.insomnia.core.api.module.Module;
import dev.joaoshiozo.insomnia.core.api.Setting.Setting;
import dev.joaoshiozo.insomnia.core.api.Setting.SettingType;
...
public class ModuleButton
{
    ...
    public ModuleButton(Module module, int x, int y, int w, int h)
    {
        ...
        int n = 0;
        for (Setting setting : InsomniaMod.settingManager.getSettings(module))
        {
    ...
    public void render(int mX, int mY)
    {
        ...
        if (module.isEnabled())
        {
            ...
            {
                switch (InsomniaMod.settingManager.getSetting("ClickGUI", "Color").getEnumValue())
                {
                    case "Purple":
                        InsomniaMod.clickGUI.drawGradient(X, Y, X + W, Y + H, new Color(140, 0, 180, 232).getRGB(), new Color(136, 0, 180, 232).getRGB());
                        break;
                    default:
                        break;
                }
            ...
            else
            {
                ...
                {
                    case "Purple":
                        InsomniaMod.clickGUI.drawGradient(X, Y, X + W , Y + H, new Color(150, 0, 180, 232).getRGB(), new Color(146, 0, 180, 232).getRGB());
                        break;
                    default:
                        break;
                }
        ...
        else
        {
            if (isHover(X, Y, W, H - 1, mX, mY))
            {
                InsomniaMod.clickGUI.drawGradient(X, Y, X + W , Y + H, new Color(220, 220, 220, 232).getRGB(), new Color(218, 218, 218, 232).getRGB());
            }
            else
            {
                InsomniaMod.clickGUI.drawGradient(X, Y, X + W, Y + H, new Color(240, 240, 240, 232).getRGB(), new Color(238, 238, 238, 232).getRGB());
            }
